  ...
  try {
    ...
    const send = async (payload: any) =>
      fetch(RESEND_API, {
        method: "POST",
        headers: {
          Authorization: `Basic ${Buffer.from(`:${RESEND_API_KEY}`).toString("base64")}`,
    ...
    if (!resAdmin.ok || !resUser.ok) {
      const ra = await resAdmin.text().catch(() => "")
      const ru = await resUser.text().catch(() => "")
      console.error("Resend errors:", ra, ru)
      return new Response(JSON.stringify({ error: "failed-to-send-emails" }), { status: 502 })
    }
